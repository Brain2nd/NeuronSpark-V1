...
## 1.1 系统级认知架构：为什么这些组件构成一个整体

以下各章节分别校验每个组件的生物学对应。但在逐一展开前，需要先建立一个**系统级的认知架构脉络**，说明这些组件不是孤立拼接的，而是遵循大脑信息处理的整体逻辑。

### 核心认知理论：互补学习系统（Complementary Learning Systems, CLS）

McClelland, McNaughton & O'Reilly (1995) 提出的 CLS 理论是理解整个架构的关键框架：

- **新皮层**（对应 SNN Decoder Layer）：缓慢学习统计规律，通过大量经验提取通用表征。局部回路、多时间尺度、层级分化。
- **海马体**（对应 SNN-Attention Layer）：快速绑定单次事件，通过联想记忆存储具体经历。稀疏门控、无衰减、内容寻址。

大脑不是只有皮层或只有海马——两者协作才能既学到通用知识又记住具体事实。我们的 3:1 混合架构正是这种协作的计算实现。
...
输入 token (感觉刺激)
  │
  ├─ Embedding (感觉编码：视/听皮层的初级特征提取)
  │
  ├─ ×3 SNN Decoder Layer (皮层局部处理)
  │   │
  │   ├─ 子层1: SNNBlock (时序特征提取)
  │   │   ├─ 因果卷积: 树突局部整合 (~4个突触延迟)
  │   │   ├─ 6条并行投影: 多受体/多调制系统的独立输入
  │   │   ├─ N=8 多β神经元: 微柱内多细胞类型的并行处理
  │   │   ├─ SelectivePLIF: 输入依赖的动态参数 (神经调制)
  │   │   └─ PonderNet: 自适应处理深度 (gamma burst内的cycle数)
  │   │
  │   ├─ 子层2: SNNFFN (非线性整合)
  │   │   └─ 双通路乘法门控: 树突的非线性计算
  │   │
  │   ├─ 侧向抑制: PV+中间神经元的divisive normalization
  │   └─ 残差连接: 跨层直接投射
  │
  ├─ ×1 SNN-Attention Layer (海马记忆系统)
  │   │
  │   ├─ 子层1: SNN-Attention
  │   │   ├─ K帧平均→token级: 从gamma时间尺度切换到theta时间尺度
  │   │   ├─ PLIFNode gate: 齿状回的新颖性检测/稀疏门控
  │   │   ├─ k·vᵀ写入M: CA3的Hebbian联想记忆编码
  │   │   ├─ q·M读出: CA1的模式补全
  │   │   └─ RoPE: 位置/网格细胞的序列位置编码
  │   │
  │   ├─ 子层2: SNNFFN (同皮层层)
...
大脑的信息处理不是单一时间尺度的——从毫秒级的突触事件到秒级的工作记忆，不同时间尺度嵌套协作。我们的架构自然复现了这种嵌套：

```
第1层 — 帧级（最快）: PLIF 膜电位动力学
  │  β∈[0.80,0.99] → τ=5~100帧
  │  类比: gamma振荡 (30-100Hz), 皮层局部回路的瞬时计算
  │
第2层 — token级（中速）: PonderNet聚合 + SNN-Attention
  │  K帧→1token, M矩阵跨token累积
  │  类比: theta节律 (4-8Hz), 海马的记忆编码/提取
  │
第3层 — 序列级（最慢）: 残差流 + M矩阵持久化
     跨整个序列的信息累积
     类比: 工作记忆, 前额叶的持续性活动
```

这不是人工设计的三层结构——三层时间尺度是从 PLIF 动力学（β衰减）、PonderNet（K帧聚合）、和残差流（层间传递）的数学性质中自然涌现的。

### 训练过程作为"发育"的证据

训练过程中观测到的参数演化（详见 Section 14）进一步证实了这是一个连贯的系统：

1. **浅层→深层的功能分化**自发涌现（V_th 深层升高更多、α 深层降低更多），对应皮层层级处理中从初级感觉到高级联合的梯度。这不是人工设定的——同样的初始化，经过数据驱动后自发分化出皮层层级结构。

2. **V_th 维度间分化**（std 涨 43-79%）对应稀疏编码的涌现——部分神经元特化为高选择性（高阈值），部分保持低选择性（低阈值），类似皮层中 grandmother cell 和 population coding 的共存。

3. **PonderNet 从 uniform 到自适应**的演化，对应大脑从"均匀注意"到"选择性注意"的发展——婴儿对所有刺激等量处理，成熟大脑快速识别简单刺激、集中资源处理复杂刺激。

4. **β 多样性的持续增强**（beta_std 上升）对应突触修剪——发育过程中初始相似的神经元通过经验逐渐分化出不同的功能角色。

这些演化模式不是独立发生的——它们是同一个学习过程的不同侧面，共同指向一个结论：**模型的拓扑结构足够接近大脑的计算原则，以至于数据驱动的优化自发复现了大脑发育中的核心现象。**
...
| N=8 个多时间尺度神经元 | 每个可见维度 8 个隐神经元，β∈[0.80,0.99] 覆盖从快速（τ≈5帧）到慢速（τ≈100帧）的时间常数谱 |
...
| K=12 上限 | 每 token 最多 12 个 SNN 时间步，PonderNet 自适应使用 1~11 步 |
...
3. **N=8 多时间尺度**：β∈[0.80,0.99] 覆盖了从 τ≈5帧到 τ≈100帧的时间常数谱。是否对应皮层微柱内不同细胞类型的时间常数分布？
...
7. **K=12 帧**：每 token 12 个 SNN 时间步，是否落在 Theta-Gamma 耦合理论预测的合理区间（7-12 cycles/theta）？
...
## 15. 待验证假设：睡眠-重播机制用于上下文增程

### 15.1 问题

SNN-Attention 层的 M 矩阵（对应海马 CA3 联想记忆）采用无衰减累积：

```
M[T] = Σ_{t≤T} gate[t] · k[t] · v[t]ᵀ
```

随着上下文增长，M 的 Frobenius 范数持续增大，大量 k·vᵀ 外积叠加导致早期和晚期模式互相干扰，content-based 分离精度下降。

这等价于一个生物学问题：**海马体的存储容量有限，如何处理持续涌入的新信息？**

### 15.2 我们的假设：计算层面的"睡眠-重播"

真实大脑通过慢波睡眠（SWS）期间的离线重播解决这个问题：
1. 清醒期：海马快速编码新事件（M 累积）
2. 睡眠期：海马记忆被重播并固化到皮层（M 压缩/转移）
3. 醒来后：海马容量恢复，可以编码新事件

我们计划在推理时模拟这个过程，实现长上下文：
...
每处理 W 个 token 后:
  U, S, V = SVD(M)
  M ← U[:, :r] · S[:r] · V[:r, :]     // 保留主要模式，丢弃噪声
```

这对应 Tononi (2006) 的突触稳态假说（Synaptic Homeostasis Hypothesis, SHY）——睡眠期间突触强度整体降低，保留信噪比高的连接，弱化噪声连接。

**请教**：SHY 假说的"突触降尺度"是全局均匀的还是选择性的？如果是选择性的，SVD 的 top-r 截断是否是一个合理的数学近似？

#### 方案 B：海马→皮层记忆巩固

```
每处理 W 个 token 后:
  summary = compress(M)                 // 从 M 提取压缩摘要
  h_residual += inject(summary)         // 注入残差流（"皮层"）
  M = 0                                 // 清空"海马"
```

这对应标准巩固理论（Standard Consolidation Theory, Squire & Alvarez 1995）——海马在清醒期快速编码情景记忆，睡眠期间通过重播将记忆逐步转移到新皮层的分布式表征中，最终海马不再需要维持该记忆。

**请教**：
1. 皮层巩固的时间尺度在真实大脑中是数天到数周。我们在推理时瞬时完成这个转移，是否在计算上可接受？还是说需要某种"渐进式"转移？
2. 巩固后的记忆在皮层中是什么形式？是改变了权重（长期记忆）还是持续性活动（工作记忆）？我们目前的方案是注入残差流（类似工作记忆的持续激活），而非修改权重。

#### 方案 C：分块处理 + 记忆接力

```
长文本分为 [chunk_1 | chunk_2 | chunk_3 | ...]

处理 chunk_1 → M_1 累积 → 睡眠 → compress(M_1) → context_1
处理 chunk_2（携带 context_1）→ M_2 累积 → 睡眠 → compress(context_1 + M_2) → context_2
处理 chunk_3（携带 context_2）→ ...
```

理论上支持无限长上下文：每一块的 M 都是"刚睡醒"的状态，容量完整。

**请教**：这种分块+接力的模式是否对应人类阅读长文档的认知策略？认知科学中是否有"分段理解→整合"的实验证据？
...
这是最精确的生物对应——睡眠期间的突触重塑（synaptic consolidation）字面意义上改变了"皮层权重"。

**请教**：
1. 推理时修改权重在工程上有灾难性遗忘的风险。大脑是否有机制防止睡眠重播导致旧记忆被覆盖？
2. 慢波睡眠的重播是精确重放还是"创造性"重组？如果是后者，可能对应某种数据增强而非精确回放。

### 15.3 训练过程中的相关观测

当前训练（17640 步）尚未实施上述机制（M 一直累积不清空），但观测到：

- **M 的有效秩**在看板中有记录（`memory/layer_XX/effective_rank`），可追踪 M 是否趋向饱和
- **gate 的写入稀疏性**由 PLIFNode 的 spike 动力学控制——高阈值（v_th=0.8）使得大部分 token 不写入 M，天然延缓了饱和
- **SNN-Attention 层的梯度份额稳定在 ~1%**，说明 M 的信息确实在被模型使用，但不是学习的主要驱动力（皮层层承担了主要学习负担）

### 15.4 期望的指导

1. **压缩机制的选择**：SVD 截断 vs 随机投影 vs 蒸馏——哪种最接近大脑的实际压缩机制？
2. **压缩触发时机**：每 W 个 token 触发一次，W 应该取多大？是否对应一个认知"段落"的长度？
3. **压缩率**：保留 top-r 个奇异值分量，r/rank(M) 的比例是否有生物学依据？
4. **皮层表征的持久性**：巩固后的 summary 应该持续多久？永久保持（像长期记忆）还是缓慢衰减（像工作记忆）？
5. **是否需要"做梦"**：重播过程是否应该包含噪声/变形（对应 REM 睡眠的梦境），而非精确回放？有假说认为 REM 期间的"创造性"重播有助于泛化。

---

## 14. 训练过程观测数据（实证佐证）

> 以下数据来自 V2.5 架构的实际预训练（1.16B 参数，17M 条中英双语数据）。
