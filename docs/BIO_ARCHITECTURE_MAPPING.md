...
模型中有两个控制时序计算量的参数，容易混淆，需明确区分：

| | K = 12（帧数） | N = 8（状态扩展） |
|---|---|---|
| 含义 | 每个 token 沿时间轴处理的 SNN 步数 | 每个可见维度并行运行的隐神经元数 |
| 方向 | 纵向（时间深度） | 横向（并行宽度） |
| 控制什么 | "想多久" — PonderNet 自适应使用 1~11 步 | "用多少种速度同时想" — 各神经元 β∈[0.80,0.99] |
| 每 token 总更新量 | K × N = 12 × 8 = 96 次神经元更新 | |
| 来源 | GPU 显存约束（原设计 K=32） | GPU 显存约束（设计 N=16） |
| 生物类比 | 一个 token 的处理周期内的时序迭代次数 | 同一微柱内不同时间常数的细胞类型数 |

两者完全正交，各自独立受显存限制，越大越好。
