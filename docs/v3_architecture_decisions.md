...
## Leaky bio-ReLU：评估后跳过（2026-04-23）

### 背景
bio-ReLU `output = max(V_pre - V_th, 0)` 存在 dying-ReLU 风险：chronically dead 神经元（跨所有 K 步+所有 batch token 永不发放）会获得严格零梯度。

### Leaky 方案
`output = max(V_pre - V_th, 0) + ε · min(V_pre - V_th, 0)`（ε≈0.01），死区也给小幅梯度。

### 量化冲突（决定性原因）
| V_pre 状态 | 非 leaky output | leaky (ε=0.01) | 稀疏性 |
|---|---|---|---|
| fire | `V_pre - V_th` > 0 | `V_pre - V_th` > 0 | 非零 |
| dead | **严格 0** | `ε·(V_pre - V_th)` 小但非零 | **破坏** |

假设死区幅度 ~0.3：
- 非 leaky：50% 位置严格 0 → 稀疏 GEMM 可跳 + 显存压缩 ~50%
- leaky：50% 位置值 ~0.003 → 稀疏率被压回 0-10%，Stage 3 稀疏 kernel 作废

### 当前 init 健康，改善不可测
- 平均 firing rate 40%（实测 median 43%）
- P(chronically dead at init) = 0.6^(K·B·T) ≈ 10^(-2M)（实际 = 0）
- 因此 "leaky 改善 %" 在未训练状态下为 **0**，只在训练漂移产生 dead 后才可见

### 决策：跳过，保留为应急介入工具
**默认：纯 bio-ReLU + 稀疏内核（稀疏是 v3 关键卖点）**

监控触发 fallback：
- 持续监控 `firing/global/health_score` + `dead_fraction`
- 若 health_score < 0.5 连续 500 step → 告警 → 临时开启 leaky（ε=0.01）热修
- 恢复后关闭，回到稀疏模式

### 天然防御机制（足够应对大多数 dying）
1. **K=12 时间步**：每 token 12 次发放尝试机会
2. **Batch 多样性**：1M+ tokens/step 的 firing pattern 多样
3. **PonderNet bias balancing**：gradient-free 调节防止 k_predictor 层塌缩
4. **Residual 流**：identity 路径保证层间梯度不被 ReLU 链截断

### 若未来真要上 leaky
需要改 4 个 Triton kernel（forward + backward × 2 variants），~80 行 Triton 代码。
backward 公式：
```
s_out = s + ε·(1-s)        # 有效输出斜率
s_post_back = (1-s)·(1-ε)  # 有效 V_post 传回斜率
grad_v_pre = g_out · s_out + total_gV · s_post_back
grad_v_th  = s_out · (total_gV - g_out)
```
