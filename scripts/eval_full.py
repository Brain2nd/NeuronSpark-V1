...
class NeuronSparkLM(LM):
    def __init__(self, checkpoint='checkpoints/ckpt_step441000',
        ...
        # v3 全部是 HF checkpoint, from_pretrained 内置 RoPE buffer 重算 + fp32 load
    ...
    def apply_chat_template(self, chat_history, add_generation_prompt: bool = True) -> str:
        # enable_thinking=True: 留空 (<|im_start|>assistant\n), 让模型自己生成 <think>...</think>
        #                       — 仅生成式评测有意义, log-lik 下 cont 概率必然偏低
        # enable_thinking=False: tokenizer 自动注入 <think>\n\n</think>\n\n, 表达 "skip thinking"
        #                        — log-lik 下落在 SFT 训练分布里, 是正确的"非 think 模式"评测
    ...
    def generate_until(self, requests):
        ...
        显式 tqdm 进度条, 因为 lm-eval 内部在 custom LM 的 generate 循环里不自动包装.
        """
        from tqdm import tqdm
        results = []
        total = len(requests)
        pbar = tqdm(requests, desc=f'generate_until [{getattr(self, "_device", "?")}]',
                    total=total, ncols=90, dynamic_ncols=False, mininterval=5.0,
