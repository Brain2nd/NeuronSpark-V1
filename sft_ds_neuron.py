"""
分布式 SFT 训练脚本：SNN 语言模型 SFT，NeuronLangevin 优化器 (DeepSpeed stage=0)
...
与 sft_ds.py 的核心差异:
  ...
  3. fp32 白名单扩到 halt_proj: 让 halt 参数走 fp32 master, 避免 bf16 量化墙.

用法:
    deepspeed --num_gpus=4 sft_ds_neuron.py \
        --pretrained_ckpt checkpoints/ckpt_step479000 \
        --sft_data_path data/mixed-sft \
        --vocab_size 64002 --D 1024 --N 8 --K 12 --num_layers 24 --D_ff 3072 \
        --batch_size 1 --accumulation_steps 8 \
        --adamw_lr 5e-5 --ortho_lr 2e-3 \
        --T_halt 1e-3 --T_ortho 5e-5 --T_plif 1e-4 \
        --dashboard_dir runs/sft_ds_neuron_<tag>
"""
...
def get_lr(it, total_iters, peak_lr, warmup_iters):
    """Cosine with warmup. 返回当前 peak_lr 下的 lr 值 (base unit)."""
    min_lr = peak_lr / 10
    if it < warmup_iters:
...
def train_epoch(epoch, model_engine, train_loader, sampler, args,
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        # 为每个 param_group 按各自 peak_lr 做 cosine schedule
        # peak 存在 group['peak_lr'] (初始化时塞进去)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SNN LM SFT (NeuronLangevin: ortho drift + Langevin diffusion)"
    )
    ...
    # 训练
    parser.add_argument("--out_dir", type=str, default="checkpoints_sft_neuron")
    ...
    parser.add_argument('--adamw_lr', type=float, default=5e-5,
    ...
    # 记录每组 peak_lr = 原始基础 lr (不含 lr_mult, 倍率由 NeuronLangevin 内部应用一次)
    # schedule: 用 cosine 把 pg['lr'] 从 0 warmup 到 peak_lr 再 decay 到 peak_lr/10;
    # step 时 NeuronLangevin 计算 effective = pg['lr'] * pg['lr_mult']
    ...
    # 强制 stage=0: NeuronLangevin 的 ortho drift 需完整 2D momentum, ZeRO≥1 会打乱
    ...
    Logger(f"\n{'='*70}")
    Logger(f"SNN LM SFT (NeuronLangevin, DeepSpeed stage=0, {world_size} GPUs)")
    ...
    Logger(f"  Precision:   bf16 + fp32 master (白名单: neuron + halt)")
