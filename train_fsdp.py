...
def wrap_model_fsdp(model, args, local_rank):
    ...
    # 自动包装策略: 每个 SNNDecoderLayer 作为独立 FSDP 单元
...
def train_epoch(epoch, model, raw_model, train_loader, sampler, optimizer, ctx, args,
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        # 梯度累积到边界步时更新
        if is_boundary:
            # Dashboard: optimizer.step() 前缓存梯度范数（集合操作）
            if dashboard:
...
if __name__ == "__main__":
    ...
    # ==================== 优化器（Adam，与 train.py / train_ddp.py 一致）====================
    ...
    Logger(f"  no_sync:     accumulation_steps={args.accumulation_steps}", rank)
    Logger(f"  Save every:  {args.save_interval} steps", rank)
    if args.dashboard_dir:
        Logger(f"  Dashboard:   {args.dashboard_dir}", rank)
